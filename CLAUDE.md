...
- **BLOBs must never reach jsonify raw**: every JSON row path (`execute_and_parse_query`, `build_rich_schema` samples, `/api/row/lookup`) routes cells through `jsonsafe_row()` in `database.py` — bytes become `<BLOB n bytes, hex ...>` placeholders (leading hex keeps file signatures readable). A new route that returns DB rows must do the same or forensic DBs (Skype, iOS) will 500 it
...
- **Stop button / abort causes**: while streaming, the Send button becomes Stop. Every abort sets `abortCause` on `state.activeStreamController` (`'user'` | `'watchdog'` | `'superseded'`); a user stop, mid-stream disconnect, or watchdog stall all keep and save the partial text and **never execute SQL from a truncated response**. In the `finally`, composer reset runs only for the stream that still owns `state.activeStreamController`; session saves are best-effort (`saveAssistantMessage()` — a failed save must never destroy a rendered answer or block SQL execution). Preserve these contracts when touching `chat.js`
- **Slow model ≠ dead connection**: the frontend watchdogs (90s pre-first-event / 120s idle) measure *transport* liveness — re-armed by any received bytes, keep-alives included. The backend pumps provider lines through a reader thread (`_iter_lines_with_keepalive` in `llm.py`) so `: keep-alive` comments flow every 15s **even during prefill**, when the provider is completely silent. Prefill of a 10k-token schema prompt can take minutes on slow hardware — never reintroduce a hard first-token deadline. The only wedged-provider guard is the stream read timeout `LLM_STREAM_READ_TIMEOUT` (default 600s, env-overridable). A pinned status notice appears after 45s without a first event
...
- **Value annotations** (`_build_value_annotations` in `llm.py`): enum-like TEXT and INTEGER columns (non-PK) get their real distinct values listed (`-- values of table.col: 'a', 'b'` / `-- values of table.col: 1, 2, 3`) inside the `<<UNTRUSTED_DATA>>` markers — guarded by the `VALUE_ANNOTATION_*` constants (≤12 distinct, ≤40 chars, tables ≤50k rows). Annotation lines are raw DB content: keep them inside the untrusted markers and routed through `_safe_sample_val()`
- **Query-outcome feedback** (`build_llm_messages` in `llm.py`): prior assistant turns carrying result metadata get an ephemeral `[Query outcome: N rows returned]` annotation when sent to the LLM (latest one also gets a ≤3-row preview inside `<<UNTRUSTED_DATA>>` markers, cells via `_safe_sample_val()`). This is context only — never an auto-retry trigger (ROADMAP.md: an empty result can be the forensically correct answer). Stored session history is never mutated
- **CTX bar semantics**: the header bar shows the *last* turn's `total_tokens` against the provider-reported context window (`get_context_window()` in `llm.py`, surfaced via `/api/provider/status` `context_length`, 60s cache). Never reintroduce cumulative summing across turns — each turn's prompt already re-contains the whole history. When the provider can't report a window, the frontend falls back to a 32k estimate and marks the tooltip `~`
...
- **Session state**: Server-side filesystem sessions with `SESSION_REFRESH_EACH_REQUEST = False` — the session file is written **only** when `session.modified` is set, so every mutation must assign the key (`session['x'] = ...`) or set `session.modified = True` explicitly. Never rely on in-place mutation of a nested object (e.g. `chat_history.append(...)`) persisting on its own: before this was disabled, the implicit every-request rewrite let the read-only status poll clobber a chat request's concurrent write and silently drop user messages (v3.17.1)
- **LLM message shape**: the first non-system message sent to a provider must be a `user` turn — qwen chat templates hard-fail on assistant-before-first-user ("No user query found in messages"). `build_llm_messages()` guarantees this by dropping leading assistant turns (covers corrupted stored histories and the `MAX_HISTORY_MESSAGES` slice landing mid-exchange); route all provider payload assembly through it
- **Provider stream errors are surfaced, never swallowed**: LM Studio streams failures as an SSE `error` frame over HTTP 200; Ollama as an `{"error": ...}` line. `stream_llm_response` converts both to `event: error`, and a stream that ends with **zero** tokens and no terminal signal is also an error — a bare `done` there renders as the app silently ignoring the user. A stream that dies after emitting tokens still ends with `done` (partial text is never discarded)
- **Grid.js table limit**: 2000 rows max for performance

## LLM Provider Setup

Supports LM Studio (OpenAI-compatible API at `localhost:1234`) and Ollama (`localhost:11434`). Configured via env vars `LLM_PROVIDER`, `LLM_API_URL`, `OLLAMA_URL`, `OLLAMA_MODEL`, `LLM_STREAM_READ_TIMEOUT` (max provider silence in seconds, default 600 — raise on very slow hardware). Provider status polled every 30 seconds from frontend.
